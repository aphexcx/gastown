...
Claude Code injects <channel source="slack" chat_id="..." sender="alice">hey mayor</channel>
...
```
Codex agent runs: gt slack reply D01EXAMPLE01 "deploy is green" --thread 1714510123.000200
...
Most inbound `message.im` events from Slack don't include the sender's display name — only the user ID (e.g., `U01EXAMPLE01`). With nothing else, the rendered `<channel>` tag's user attribute showed the cryptic ID:

```
← gt-slack · U01EXAMPLE01: hi
```
