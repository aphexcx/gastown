...
func HandleReply(_ context.Context, townRoot, senderAddress string, args ReplyArgs) (ReplyResult, error) {
	...
	// Validate each file path matches the CLI's gt slack reply rules so
	// MCP-tool failures surface immediately to the agent (as a tool error)
	// instead of as a silent partial publish minutes later.
	for _, p := range args.Files {
		if !filepath.IsAbs(p) {
			return ReplyResult{Error: fmt.Sprintf("file path %q must be absolute", p)}, nil
		}
		info, err := os.Stat(p)
		if err != nil {
			return ReplyResult{Error: fmt.Sprintf("file %s: %v", p, err)}, nil
		}
		if info.IsDir() {
			return ReplyResult{Error: fmt.Sprintf("file %s is a directory", p)}, nil
		}
	}

	dir := filepath.Join(townRoot, constants.DirRuntime, "slack_outbox")
	// Copy the slice to avoid aliasing the caller's input.
	msg := &OutboxMessage{
		From:      senderAddress,
		ChatID:    args.ChatID,
		Text:      args.Text,
