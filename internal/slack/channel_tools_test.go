...
func TestHandleReply_FilesRoundTrip(t *testing.T) {
	...
	// HandleReply now stats each file (parity with `gt slack reply --files`),
	// so the test must point at real files.
	foo := filepath.Join(t.TempDir(), "foo.png")
	bar := filepath.Join(t.TempDir(), "bar.jpg")
	...
	if len(parsed.Files) != 2 || parsed.Files[0] != foo || parsed.Files[1] != bar {
		t.Fatalf("Files = %v, want [%s %s]", parsed.Files, foo, bar)
	}
}

func TestHandleReply_RejectsMissingFiles(t *testing.T) {
	town := t.TempDir()
	dir := filepath.Join(town, constants.DirRuntime, "slack_outbox")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	...
	out, err := HandleReply(context.Background(), town, "gastown/crew/cog", ReplyArgs{
		ChatID: "D1",
		Text:   "see attached",
		Files:  []string{"/nonexistent/missing.png"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if out.OK {
		t.Fatalf("OK=true with missing file; want OK=false (result=%+v)", out)
	}
	if !strings.Contains(out.Error, "missing.png") {
		t.Errorf("error should mention the file; got %q", out.Error)
	}
}

func TestHandleReply_RejectsRelativeFiles(t *testing.T) {
	town := t.TempDir()
	dir := filepath.Join(town, constants.DirRuntime, "slack_outbox")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	...
	out, err := HandleReply(context.Background(), town, "gastown/crew/cog", ReplyArgs{
		ChatID: "D1",
		Text:   "see attached",
		Files:  []string{"relative/path.png"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if out.OK {
		t.Fatalf("OK=true with relative file path; want OK=false (result=%+v)", out)
	}
	if !strings.Contains(out.Error, "absolute") {
		t.Errorf("error should mention 'absolute'; got %q", out.Error)
	}
}
