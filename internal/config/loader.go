...
func BuildStartupCommandWithAgentOverride(envVars map[string]string, rigPath, prompt, agentOverride string) (string, error) {
	...
	// Auto-inject --channels plugin:gt-slack@gastown for Claude agents when
	// slack.json's channels_enabled is true. Must fire on ALL resolution
	// paths (including agent overrides), or polecats launched with --agent
	// silently skip the channels plugin.
	maybeInjectClaudeChannels(rc)
